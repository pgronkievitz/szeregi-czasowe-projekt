...
\@ref(fig:decomposedkomunikacjaA) dekompozycja dla komunikacji dała od razu dużo
...
```{r wydatkiBoxCox, fig.cap="Wykres wydatków po transformacji Boxa-Coxa"}
...
jest lag równy 5 dla wydatków (\@ref(fig:wydatkiBoxCox)), pozostałe są
...
wartość (\@ref(fig:komunikacjaBoxCox)) - znaczy to, że na ten moment nie
...
Jak widać na rysunku \@ref(naiveforecast) metoda naiwna daje znacznie lepsze
...
Metoda z rysunku \@ref(fig:snaive) jest nieco gorsza od prognozowania na podstawie
poprzedniego pomiaru, ponieważ trend ma dużo większe znaczenie w tym przypadku
niż sezonowość.
